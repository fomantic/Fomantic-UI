...
function buildTS(src, type, config, callback) {
    if (!install.isSetup()) {
        console.error('Cannot build Typescript. Run "gulp install" to set-up Semantic');
        callback();
    ...
    if (callback === undefined) {
        ...
        config = type;
        type = src;
        src = config.paths.source.definitions + '/**/' + config.globs.components + (config.globs.ignored || '') + '.ts';
    }
    ...
    if (globs.individuals !== undefined && typeof src === 'string') {
        ...
        src = config.paths.source.definitions + '/**/{' + components + '}' + (config.globs.ignored || '') + '.ts';
    }
...
module.exports.watch = function (type, config) {
    gulp
        .watch([normalize(config.paths.source.definitions + '/**/*.ts')])
        .on('all', function (event, path) {
            ...
            // Update timeout
            timeout = setTimeout(() => {
                console.log('Change in typescript detected');
                // Copy files to build in another array
                const buildFiles = [...files];
                // Call method
                gulp.series((callback) => buildTS(buildFiles, type, config, callback))();
...
module.exports.buildTS = buildTS;
